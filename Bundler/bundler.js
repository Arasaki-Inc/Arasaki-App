...
async function minifyTypescript(item, proc_dirname, proc_dirname_dev, bundle)
{
    if (needsCaching(item.path, 'ts') || needsProcessing(item.path))
    {
        try
        {
            ...
            execSync('npx tsc ' + (bundle ? join(proc_dirname_dev, 'ts', 'core.ts') : item.path) + ' --outFile "' + output + '" --target ES2021 --lib DOM,ES2021,WebWorker --module amd --esModuleInterop --allowSyntheticDefaultImports --forceConsistentCasingInFileNames --strict --skipLibCheck',
                err =>
            {
                if (err)
                {
