...
using Arasaki.Machine;

public static class Logger
{
    private static Serilog.Core.Logger InternalConsoleLogger;
    public static string DefaultLogFormat { get; } = "[{Timestamp:yyyy-MM-dd HH:mm:ss.fff} {Level:u3}] {Message}{NewLine}{Exception}";

    public static void Initialise(Serilog.Core.Logger logger)
    {
        try
        {
            InternalConsoleLogger = logger;
            if (!Runtime.IsWASM && OS.IsWindows) Console.BufferWidth = Console.WindowWidth;
            ClearBuffer();
        }
